...
class Form(CloudResource):
	...
	def __init__(self, form_id, user_id, site_id, data=None):
		...
		self._endpoint = "user/{0}/site/{1}/form/{2}".format(self.user_id, self.site_id, self.form_id)
	...
	def list_form_entries(self, **filters):
		...
		return PaginatedList(result, FormEntry, (self.user_id, self.site_id, self.form_id), "form_entry_id")
